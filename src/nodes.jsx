...
function NodeEarth({ size = 200, lat = 40.7, lon = -74.0, date = new Date(), onDrag }) {
  ...
  const L_earth = earthHeliocentricLongitude(date); // radians
  const lam_sun = L_earth + Math.PI; // geocentric sun longitude, radians
  const obliq = trueObliquity(date); // corrected for precession + nutation
  ...
  const sub_lon_deg = subSolarLongitude(date);
  ...
  return (
    ...
    >
      ...
      <g transform={`translate(${cx}, ${cy})`}>
        ...
        {(() => {
          const moonEq = moonEquatorialPosition(date);
          const moonSky = moonAltAz(date, lat, lon);
...
function NodeSolarSystem({ size = 240, date = new Date(), lat = 0, onDragDate }) {
  ...
  const earthLon = earthHeliocentricLongitude(date);
...
export { NodeHumans, NodeEarth, NodeSolarSystem, NodeGalaxy, NodeSupercluster, NodeUniverse };
